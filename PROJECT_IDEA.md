...
**Current app version: `0.15.2`** (SemVer in `app/package.json` / [CHANGELOG.md](CHANGELOG.md)). Nine
...
nothing runs past the right edge and the page never scrolls sideways; tablet-and-up is unchanged. Runs on the Forge platform at **`0.26.1`**
(control + data-plane, digest-pinned; the `./forge` wrapper dials the in-container API on the IPv4 literal
`127.0.0.1` so it never misdials IPv6 `::1` — **P20**). **Deploys are now ONE command — `forge release`
